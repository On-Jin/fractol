...
void			buddhabrot_color(V_PRECI c_r, V_PRECI c_i, int i,
									int height_bud, int width_bud,
									t_tool *tool, t_bud *bud,
		...
		t_bud bud)
{
	...
	i = 0;
	c_r = (((V_PRECI)x) / ((V_PRECI)height_bud * 1.125)) *
				((V_PRECI)tool.xmax - (V_PRECI)tool.xmin) + tool.xmin;
	c_i = (((V_PRECI)y) / (V_PRECI)height_bud) *
	...
	{
		if (i < tool.iter)
			buddhabrot_color(c_r, c_i, i, height_bud, width_bud,
								  &tool, &bud, out, out2, out3);
	}
	else
	{
		if (i == tool.iter)
